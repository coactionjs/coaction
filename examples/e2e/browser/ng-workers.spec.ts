...
const connectWorker = (
  ...
  page.evaluate(
    ({ kind, name, expectedCount: nextExpectedCount }) =>
      window.__ngWorkerHarness.connect({
        kind,
        name,
...
const addAsyncInWorker = (
  ...
  page.evaluate(
    ({ kind, name, step: nextStep }) =>
      window.__ngWorkerHarness.addAsync({
        kind,
        name,
        step: nextStep
      }),
...
const waitForWorkerCount = (
  ...
  page.evaluate(
    ({ kind, name, expectedCount }) =>
      window.__ngWorkerHarness.waitForCount({
        kind,
        name,
        expectedCount
      }),
...
const disconnectWorker = (page: Page, kind: 'shared' | 'web', name: string) =>
  page.evaluate(
    ({ kind, name }) =>
      window.__ngWorkerHarness.disconnect({
        kind,
        name
      }),
    { kind, name }
...
test.afterEach(async ({ page }) => {
  await page.evaluate(async () => {
    if ('__ngWorkerHarness' in window) {
      await window.__ngWorkerHarness.disconnectAll();
    }
  });
});

test('ng shared worker: cross-page sync propagates through the signal accessor', async ({
  browser
}) => {
  const name = createScenarioName('ng-shared-sync');
...
test('ng web worker: async action + error redaction through the promise-returning proxy', async ({
  page
}) => {
  const name = createScenarioName('ng-web-actions');
  const secretMessage = 'ng-worker-error';

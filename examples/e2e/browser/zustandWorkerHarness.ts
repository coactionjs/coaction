...
type WorkerClient = {
  kind: WorkerKind;
  name: string;
  store: AsyncStore<ZustandWorkerCounterState>;
  zustandStore: StoreApi<ZustandWorkerCounterState>;
...
export type ZustandWorkerHarness = {
...
const waitFor = async (
  ...
  throw new Error('Timed out waiting for zustand worker scenario condition.');
};
...
const getWorkerUrl = (name: string) => {
  const url = new URL('./zustand.worker.ts', import.meta.url);
...
const ensureClient = async (
  ...
  const zustandStore = createZustandCounterStore();
  const store = create<ZustandWorkerCounterState>(() => adapt(zustandStore), {
    name: options.name,
    worker
  });
...
// The read path that's unique to a binder adapter: the underlying zustand
// store must reflect the synced value too, not just coaction's snapshot.
const readUnderlyingCount = (client: WorkerClient) =>
  client.zustandStore.getState().count;
...
const fail = async (
  ...
  throw new Error('Expected zustand worker action to fail.');
};
...
export const createZustandWorkerHarness = (): ZustandWorkerHarness => ({
  connect,
  add,
  addAsync,
  fail,

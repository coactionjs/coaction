...
type WorkerClient = {
  kind: WorkerKind;
  name: string;
  store: AsyncStore<ValtioWorkerCounterState>;
  valtioProxy: ValtioWorkerCounterState;
...
export type ValtioWorkerHarness = {
...
const waitFor = async (
  ...
  throw new Error('Timed out waiting for valtio worker scenario condition.');
};
...
const getWorkerUrl = (name: string) => {
  const url = new URL('./valtio.worker.ts', import.meta.url);
...
const ensureClient = async (
  ...
  const valtioProxy = createValtioCounterStore();
  const store = create<ValtioWorkerCounterState>(() => adapt(valtioProxy), {
    name: options.name,
    worker
  });
...
// The read path that's unique to a binder adapter: the underlying valtio
// proxy itself must reflect the synced value too, not just coaction's
// snapshot.
const readUnderlyingCount = (client: WorkerClient) => client.valtioProxy.count;
...
const fail = async (
  ...
  throw new Error('Expected valtio worker action to fail.');
};
...
export const createValtioWorkerHarness = (): ValtioWorkerHarness => ({
  connect,
  add,
  addAsync,
  fail,

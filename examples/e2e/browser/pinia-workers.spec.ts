...
const connectWorker = (
  ...
  page.evaluate(
    ({ kind, name, expectedCount: nextExpectedCount }) =>
      window.__piniaWorkerHarness.connect({
        kind,
        name,
...
const addInWorker = (
  ...
  page.evaluate(
    ({ kind, name, step: nextStep }) =>
      window.__piniaWorkerHarness.add({
        kind,
        name,
        step: nextStep
      }),
...
const addAsyncInWorker = (
  ...
  page.evaluate(
    ({ kind, name, step: nextStep }) =>
      window.__piniaWorkerHarness.addAsync({
        kind,
        name,
        step: nextStep
      }),
...
const failInWorker = (
  ...
  page.evaluate(
    ({ kind, name, message: nextMessage }) =>
      window.__piniaWorkerHarness.fail({
        kind,
        name,
        message: nextMessage
      }),
...
const waitForWorkerCount = (
  ...
  page.evaluate(
    ({ kind, name, expectedCount }) =>
      window.__piniaWorkerHarness.waitForCount({
        kind,
        name,
        expectedCount
      }),
...
const disconnectWorker = (page: Page, kind: 'shared' | 'web', name: string) =>
  page.evaluate(
    ({ kind, name }) =>
      window.__piniaWorkerHarness.disconnect({
        kind,
        name
      }),
    { kind, name }
...
test.afterEach(async ({ page }) => {
  await page.evaluate(async () => {
    if ('__piniaWorkerHarness' in window) {
      await window.__piniaWorkerHarness.disconnectAll();
    }
  });
});

test('pinia shared worker: cross-page sync reaches the bound pinia store', async ({
  browser
}) => {
  const name = createScenarioName('pinia-shared-sync');
  ...
  // Unique to a binder adapter: the underlying pinia store instance itself
  // (not just coaction's snapshot) must reflect the value synced from the
  // worker.
...
test('pinia web worker: async action + error redaction through the promise-returning proxy', async ({
  page
}) => {
  const name = createScenarioName('pinia-web-actions');
  const secretMessage = 'pinia-worker-error';

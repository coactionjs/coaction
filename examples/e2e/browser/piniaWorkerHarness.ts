...
import {
  createPiniaWorkerCounter,
  type PiniaWorkerCounterState
} from './piniaWorkerCounter';
...
type WorkerClient = {
  kind: WorkerKind;
  name: string;
  store: AsyncStore<PiniaWorkerCounterState>;
  piniaStore: PiniaWorkerCounterState;
...
export type PiniaWorkerHarness = {
...
const waitFor = async (
  ...
  throw new Error('Timed out waiting for pinia worker scenario condition.');
};
...
const getWorkerUrl = (name: string) => {
  const url = new URL('./pinia.worker.ts', import.meta.url);
...
const ensureClient = async (
  ...
  const { factory, getInstance } = createPiniaWorkerCounter(options.name);
  const store = create<PiniaWorkerCounterState>(factory, {
    name: options.name,
    worker
  });
  const piniaStore = getInstance();
...
// The read path that's unique to a binder adapter: the underlying pinia
// store instance itself must reflect the synced value too, not just
// coaction's snapshot.
const readUnderlyingCount = (client: WorkerClient) => client.piniaStore.count;
...
const fail = async (
  ...
  throw new Error('Expected pinia worker action to fail.');
};
...
export const createPiniaWorkerHarness = (): PiniaWorkerHarness => ({
  connect,
  add,
  addAsync,
  fail,

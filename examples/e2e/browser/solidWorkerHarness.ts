import { create } from '../../../packages/coaction-solid/src/index';
import type { StoreWithAsyncFunction } from '../../../packages/coaction-solid/src/index';
...
export type SolidWorkerHarness = {
...
const waitFor = async (
  ...
  throw new Error('Timed out waiting for solid worker scenario condition.');
};
...
const ensureClient = async (
  ...
  const countAccessor = useStore((state) => state.count);
...
const readCount = (client: WorkerClient) => client.countAccessor();
...
const fail = async (
  ...
  throw new Error('Expected solid worker action to fail.');
};
...
export const createSolidWorkerHarness = (): SolidWorkerHarness => ({
  connect,
  add,
  addAsync,
  fail,

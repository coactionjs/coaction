import { create } from '../../../packages/coaction-ng/src/index';
import type { StoreWithAsyncFunction } from '../../../packages/coaction-ng/src/index';
...
type WorkerClient = {
  kind: WorkerKind;
  name: string;
  store: StoreWithAsyncFunction<WorkerCounterState>;
...
const waitFor = async (
  ...
  throw new Error('Timed out waiting for ng worker scenario condition.');
};
...
const readCount = (client: WorkerClient) => client.countSignal();
...
const fail = async (
  ...
  throw new Error('Expected ng worker action to fail.');
};
...
export const createNgWorkerHarness = (): NgWorkerHarness => ({
  connect,
  add,
  addAsync,
  fail,

...
import {
  createJotaiWorkerCounter,
  type JotaiWorkerCounterState
} from './jotaiWorkerCounter';
...
type WorkerClient = {
  kind: WorkerKind;
  name: string;
  store: AsyncStore<JotaiWorkerCounterState>;
  readUnderlying: () => number;
...
export type JotaiWorkerHarness = {
...
const waitFor = async (
  ...
  throw new Error('Timed out waiting for jotai worker scenario condition.');
};
...
const getWorkerUrl = (name: string) => {
  const url = new URL('./jotai.worker.ts', import.meta.url);
...
const ensureClient = async (
  ...
  const { factory, jotaiStore, countAtom } = createJotaiWorkerCounter();
  const store = create<JotaiWorkerCounterState>(factory, {
    name: options.name,
    worker
  });
  const client: WorkerClient = {
    ...
    name: options.name,
    store,
    readUnderlying: () => jotaiStore.get(countAtom),
    worker
  };
...
// The read path that's unique to a binder adapter: the underlying jotai
// store/atom itself must reflect the synced value too, not just coaction's
// snapshot.
const readUnderlyingCount = (client: WorkerClient) => client.readUnderlying();
...
const fail = async (
  ...
  throw new Error('Expected jotai worker action to fail.');
};
...
export const createJotaiWorkerHarness = (): JotaiWorkerHarness => ({
  connect,
  add,
  addAsync,
  fail,

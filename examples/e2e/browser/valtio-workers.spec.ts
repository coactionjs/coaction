...
const connectWorker = (
  ...
  page.evaluate(
    ({ kind, name, expectedCount: nextExpectedCount }) =>
      window.__valtioWorkerHarness.connect({
        kind,
        name,
...
const addInWorker = (
  ...
  page.evaluate(
    ({ kind, name, step: nextStep }) =>
      window.__valtioWorkerHarness.add({
        kind,
        name,
        step: nextStep
      }),
...
const addAsyncInWorker = (
  ...
  page.evaluate(
    ({ kind, name, step: nextStep }) =>
      window.__valtioWorkerHarness.addAsync({
        kind,
        name,
        step: nextStep
      }),
...
const failInWorker = (
  ...
  page.evaluate(
    ({ kind, name, message: nextMessage }) =>
      window.__valtioWorkerHarness.fail({
        kind,
        name,
        message: nextMessage
      }),
...
const waitForWorkerCount = (
  ...
  page.evaluate(
    ({ kind, name, expectedCount }) =>
      window.__valtioWorkerHarness.waitForCount({
        kind,
        name,
        expectedCount
      }),
...
const disconnectWorker = (page: Page, kind: 'shared' | 'web', name: string) =>
  page.evaluate(
    ({ kind, name }) =>
      window.__valtioWorkerHarness.disconnect({
        kind,
        name
      }),
    { kind, name }
...
test.afterEach(async ({ page }) => {
  await page.evaluate(async () => {
    if ('__valtioWorkerHarness' in window) {
      await window.__valtioWorkerHarness.disconnectAll();
    }
  });
});

test('valtio shared worker: cross-page sync reaches the bound proxy', async ({
  browser
}) => {
  const name = createScenarioName('valtio-shared-sync');
  ...
  // Unique to a binder adapter: the underlying valtio proxy itself (not just
  // coaction's snapshot) must reflect the value synced from the worker.
...
test('valtio web worker: async action + error redaction through the promise-returning proxy', async ({
  page
}) => {
  const name = createScenarioName('valtio-web-actions');
  const secretMessage = 'valtio-worker-error';

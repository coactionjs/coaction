...
const connectWorker = (
  ...
  page.evaluate(
    ({ kind, name, expectedCount: nextExpectedCount }) =>
      window.__jotaiWorkerHarness.connect({
        kind,
        name,
...
const addInWorker = (
  ...
  page.evaluate(
    ({ kind, name, step: nextStep }) =>
      window.__jotaiWorkerHarness.add({
        kind,
        name,
        step: nextStep
      }),
...
const addAsyncInWorker = (
  ...
  page.evaluate(
    ({ kind, name, step: nextStep }) =>
      window.__jotaiWorkerHarness.addAsync({
        kind,
        name,
        step: nextStep
      }),
...
const failInWorker = (
  ...
  page.evaluate(
    ({ kind, name, message: nextMessage }) =>
      window.__jotaiWorkerHarness.fail({
        kind,
        name,
        message: nextMessage
      }),
...
const waitForWorkerCount = (
  ...
  page.evaluate(
    ({ kind, name, expectedCount }) =>
      window.__jotaiWorkerHarness.waitForCount({
        kind,
        name,
        expectedCount
      }),
...
const disconnectWorker = (page: Page, kind: 'shared' | 'web', name: string) =>
  page.evaluate(
    ({ kind, name }) =>
      window.__jotaiWorkerHarness.disconnect({
        kind,
        name
      }),
    { kind, name }
...
test.afterEach(async ({ page }) => {
  await page.evaluate(async () => {
    if ('__jotaiWorkerHarness' in window) {
      await window.__jotaiWorkerHarness.disconnectAll();
    }
  });
});

test('jotai shared worker: cross-page sync reaches the bound atom', async ({
  browser
}) => {
  const name = createScenarioName('jotai-shared-sync');
  ...
  // Unique to a binder adapter: the underlying jotai store/atom itself (not
...
test('jotai web worker: async action + error redaction through the promise-returning proxy', async ({
  page
}) => {
  const name = createScenarioName('jotai-web-actions');
  const secretMessage = 'jotai-worker-error';

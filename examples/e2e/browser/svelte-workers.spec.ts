...
const connectWorker = (
  ...
  page.evaluate(
    ({ kind, name, expectedCount: nextExpectedCount }) =>
      window.__svelteWorkerHarness.connect({
        kind,
        name,
...
const addInWorker = (
  ...
  page.evaluate(
    ({ kind, name, step: nextStep }) =>
      window.__svelteWorkerHarness.add({
        kind,
        name,
        step: nextStep
      }),
...
const addAsyncInWorker = (
  ...
  page.evaluate(
    ({ kind, name, step: nextStep }) =>
      window.__svelteWorkerHarness.addAsync({
        kind,
        name,
        step: nextStep
      }),
...
const failInWorker = (
  ...
  page.evaluate(
    ({ kind, name, message: nextMessage }) =>
      window.__svelteWorkerHarness.fail({
        kind,
        name,
        message: nextMessage
      }),
...
const waitForWorkerCount = (
  ...
  page.evaluate(
    ({ kind, name, expectedCount }) =>
      window.__svelteWorkerHarness.waitForCount({
        kind,
        name,
        expectedCount
      }),
...
const disconnectWorker = (page: Page, kind: 'shared' | 'web', name: string) =>
  page.evaluate(
    ({ kind, name }) =>
      window.__svelteWorkerHarness.disconnect({
        kind,
        name
      }),
    { kind, name }
...
test.afterEach(async ({ page }) => {
  await page.evaluate(async () => {
    if ('__svelteWorkerHarness' in window) {
      await window.__svelteWorkerHarness.disconnectAll();
    }
  });
});

test('svelte shared worker: cross-page sync propagates through the readable store', async ({
  browser
}) => {
  const name = createScenarioName('svelte-shared-sync');
  ...
  // Subscribing fired synchronously with the already-synced value.
  ...
  // Page A never called add() itself: the readable store only logs 3 if the
  // SharedWorker's remote update pushed through svelte's own subscribe contract.
...
test('svelte web worker: async action + error redaction through the promise-returning proxy', async ({
  page
}) => {
  const name = createScenarioName('svelte-web-actions');
  const secretMessage = 'svelte-worker-error';

...
import { create } from '../../../packages/coaction-vue/src/index';
import type { StoreWithAsyncFunction } from '../../../packages/coaction-vue/src/index';
...
const waitFor = async (
  ...
  throw new Error('Timed out waiting for vue worker scenario condition.');
};
...
const ensureClient = async (
  ...
  const log: number[] = [];
  const countRef = scope.run(() => {
    const ref = useStore((state) => state.count);
    watchEffect(() => {
      log.push(ref.value);
    });
    return ref;
...
const readCount = (client: WorkerClient) => client.countRef.value;
...
const fail = async (
  ...
  throw new Error('Expected vue worker action to fail.');
};
...
export const createVueWorkerHarness = (): VueWorkerHarness => ({
  connect,
  add,
  addAsync,
  fail,

...
const connectWorker = (
  ...
  page.evaluate(
    ({ kind, name, expectedCount: nextExpectedCount }) =>
      window.__solidWorkerHarness.connect({
        kind,
        name,
...
const addInWorker = (
  ...
  page.evaluate(
    ({ kind, name, step: nextStep }) =>
      window.__solidWorkerHarness.add({
        kind,
        name,
        step: nextStep
      }),
...
const addAsyncInWorker = (
  ...
  page.evaluate(
    ({ kind, name, step: nextStep }) =>
      window.__solidWorkerHarness.addAsync({
        kind,
        name,
        step: nextStep
      }),
...
const failInWorker = (
  ...
  page.evaluate(
    ({ kind, name, message: nextMessage }) =>
      window.__solidWorkerHarness.fail({
        kind,
        name,
        message: nextMessage
      }),
...
const waitForWorkerCount = (
  ...
  page.evaluate(
    ({ kind, name, expectedCount }) =>
      window.__solidWorkerHarness.waitForCount({
        kind,
        name,
        expectedCount
      }),
...
const disconnectWorker = (page: Page, kind: 'shared' | 'web', name: string) =>
  page.evaluate(
    ({ kind, name }) =>
      window.__solidWorkerHarness.disconnect({
        kind,
        name
      }),
    { kind, name }
...
test.afterEach(async ({ page }) => {
  await page.evaluate(async () => {
    if ('__solidWorkerHarness' in window) {
      await window.__solidWorkerHarness.disconnectAll();
    }
  });
});

test('solid shared worker: cross-page sync propagates through the signal accessor', async ({
  browser
}) => {
  const name = createScenarioName('solid-shared-sync');
...
test('solid web worker: async action + error redaction through the promise-returning proxy', async ({
  page
}) => {
  const name = createScenarioName('solid-web-actions');
  const secretMessage = 'solid-worker-error';

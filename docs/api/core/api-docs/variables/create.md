...
- When an object input only contains functions, prefer explicit `sliceMode` to avoid ambiguous inference.
- When `clientTransport` or `worker` is provided, returned store methods become promise-returning methods because execution happens on the main shared store.
- New semantics should prefer explicit helpers or variants over adding more ambiguous `create()` input forms.

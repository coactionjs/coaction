...
| Authorization     | Unknown actions and denied requests never execute; unexpected errors are redacted by default, and only an explicit `mapError` result becomes client-visible.                                              |
| Convergence       | Contiguous updates apply once; duplicates are ignored; gaps and reconnects full-sync; superseded-authority action responses reject with an unknown outcome and never roll the mirror back.                |
